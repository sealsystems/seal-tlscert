...
tlscert.get = function (env) {
  ...
  if (!directory) {
    ...
    log.warn('Using selfsigned certifiacte. TLS checks disabled.');
  }
  ...
  Object.keys(types).forEach((type) => {
    try {
      ...
      result[type] = fs.readFileSync(path.join(directory, `${type}.pem`)).toString('utf8');
